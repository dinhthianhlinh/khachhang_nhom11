...
public class SearchSanPhamActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_search_san_pham);
    ...
    void setupSearchRecyclerView(String searchTerm){
        ...
        Query query = Utility.getCollectionReference()
                .whereGreaterThanOrEqualTo("tenSP",searchTerm)
                .whereLessThanOrEqualTo("tenSP",searchTerm+'\uf8ff');

        FirestoreRecyclerOptions<SanPham> options = new FirestoreRecyclerOptions.Builder<SanPham>()
                .setQuery(query,SanPham.class).build();

        adapter = new SanPhamAdapter(options,getApplicationContext());
